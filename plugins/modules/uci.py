...
def uci_delete(module, binary, item, key, val=None):
    ...
    if module.check_mode:
        module.exit_json(changed=True, msg="Would delete {}".format(key))
...
def uci_set(module, binary, item, key, value, noreturn=False, current_value=None):
    if not value:
    ...
    if module.check_mode:
        if not noreturn:
            ...
            if module._diff:
                diff = {
                    'before': "{} = {}\n".format(key, current_value if current_value else "(not set)"),
                    'after': "{} = {}\n".format(key, value)
                }
            module.exit_json(changed=True, diff=diff, msg="Would set {}='{}'".format(key, value))
        return
    ...
    if not key:
        ...
        if not sval:
            ...
            if create:
                ...
                uci_set(module, bin_path, item, skey, type, current_value=sval)
            else:
        ...
        if val:
            ...
            if is_set(item, val, value):
                ...
                uci_set(module, bin_path, item, key, value, current_value=val)
            ...
            if not type:
                module.fail_json(msg="Is necessary to create section but type wasn't specified")
            uci_set(module, bin_path, item, skey, type, noreturn=True, current_value=sval)
            uci_set(module, bin_path, item, key, value, current_value=val)
